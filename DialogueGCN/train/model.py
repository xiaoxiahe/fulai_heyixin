...
class MaskedEdgeAttention(nn.Module):
    ...
    def forward(self, M, lengths, edge_ind):
        ...
        if attn_type == 'attn1':
            ...
            # alpha的维度: (batch_size, max_seq_len, seq_len)
            batch_size, dim1, dim2 = alpha.size()
            ...
            for i, j in enumerate(edge_ind):
                ...
                for x in j:
                    # 添加严格的边界检查
                    if (isinstance(x, (list, tuple)) and len(x) >= 2 and
                        isinstance(x[0], (int, np.integer)) and isinstance(x[1], (int, np.integer)) and
                        0 <= int(x[0]) < dim1 and 0 <= int(x[1]) < dim2):
                        valid_edges.append((i, int(x[0]), int(x[1])))
            
            # 批量设置mask值，这样更安全
            for i, x0, x1 in valid_edges:
                mask[i, x0, x1] = 1
                mask_copy[i, x0, x1] = 1
...
def classify_node_features(emotions, seq_lengths, umask, matchatt_layer, linear_layer, dropout_layer, smax_fc_layer, nodal_attn, avec, no_cuda):
    ...
    if nodal_attn:
        ...
        if avec:
            # 添加边界检查
            max_seq_len, batch_size, _ = hidden.size()
            hidden_list = []
            for j in range(len(seq_lengths)):
                if j < batch_size:
                    actual_len = min(seq_lengths[j], max_seq_len)
                    hidden_list.append(hidden[:actual_len, j, :])
            return torch.cat(hidden_list) if hidden_list else hidden

        log_prob = F.log_softmax(hidden, 2)
        # 添加边界检查，防止索引越界
        # log_prob 形状: (max_seq_len, batch_size, n_classes)
        max_seq_len, batch_size, n_classes = log_prob.size()
        log_prob_list = []
        for j in range(len(seq_lengths)):
            if j < batch_size:
                # 确保seq_lengths[j]不超过max_seq_len
                actual_len = min(seq_lengths[j], max_seq_len)
                log_prob_list.append(log_prob[:actual_len, j, :])
        log_prob = torch.cat(log_prob_list) if log_prob_list else log_prob
        return log_prob
...
class GraphNetwork(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_norm, edge_type, seq_lengths, umask, nodal_attn, avec):
        ...
        # torch-geometric RGCNConv API: edge_type和edge_norm作为位置参数或关键字参数
